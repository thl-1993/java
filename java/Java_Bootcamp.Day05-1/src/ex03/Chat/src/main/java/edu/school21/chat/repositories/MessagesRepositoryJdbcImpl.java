...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    private static final String UPDATE_MESSAGE = "UPDATE chat.Message SET text = ?, date_time = ?, author = ?, room = ? WHERE id = ?";
    ...
    public void update(Message message) {
        if (message.getId() == null) {
            throw new IllegalArgumentException("Message ID cannot be null for an update");
        }
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            if (rowsUpdated == 0) {
                throw new RuntimeException("Message with ID " + message.getId() + " not found in the database");
            }
        } catch (SQLException e) {
            throw new RuntimeException("Failed to update the message: " + e.getMessage(), e);
        }
    }
}